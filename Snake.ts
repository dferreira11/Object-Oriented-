...
export default class Snake implements ICollidable {
  ...
  //Constructs a snake with a specified color@param color - the color of the snake
  ...
  public grow() {
    ...
    this.currentParts__.push(newPoint); // Adds the new body part to the Snake
  }
  //Moves the snake in its current direction by the specified number of squares.@param squares - The number of squares to move.
  ...
  public didCollideWithObject(other: ICollidable): boolean {
    if (!(other instanceof Snake)) {
      ...
      if (this === otherSnake) {
        // Checks for a collision with itself, like head with bottom, or any other part of the body
        return this.currentParts__
          .slice(1)
          .some((part) => part.equals(this.head));
      } else {
        // Checks for a possible collision with another snake
